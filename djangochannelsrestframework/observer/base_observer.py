...
class BaseObserver:
    ...
    The decorators `@model_observer` and
    `@observer` replaced the wrapped method with an instance of these classes.
    You can then access the methods of this class
    using the method name that you wrapped.
    """
    ...
    def serializer(self, func):
        ...
        The method that this wraps is evaluated just
        after the observer is triggered
        before the result is sent over the channel layer.
        That means you **DO NOT** have access to user or other request information.

        The result of this method is what is sent over the channel layer.
        If you need to modify that with user specific information
        then you need to do that in the observer handler method.
        ...
        The advantage of doing serialization
        at this point is that it happens only once even if 1000s of consumers are
    ...
    async def subscribe(
        ...
        args and kwargs passed here are provided
        to the :meth:`groups_for_consumer` method to enable custom
        partitioning of events.

        If the request_id is passed to the subscribe method
        then the observer will track that request id and provide it
    ...
    async def unsubscribe(
        ...
        If the request_id is passed to the un-subscribe method
        then this will un-subscribe the requests with the same
        id that called the :meth:`subscribe` method.
        If no `request_id` is provided then all subscribed requests for
    ...
    def group_names_for_consumer(
        ...
        if self._group_names_for_consumer:
            for group in self._group_names_for_consumer(
                ...
                yield self.clean_group_name(f'{self._stable_observer_id}-{group}')
            return
    ...
    def groups_for_consumer(
        ...
        """
        .. note::
            Should be used as a method decorator
            eg: `@observed_handler.groups_for_consumer`


        The decorated method is used
        when :meth:`subscribe` and :meth:`unsubscribe` are called to enumerate the
        corresponding groups to un/subscribe to.

        The `args` and `kwargs` providing
        to :meth:`subscribe` and :meth:`unsubscribe` are passed here to enable this.

        .. code-block:: python

                @classroom_change_handler.groups_for_consumer
                def classroom_change_handler(
                    self,
                    school=None,
                    classroom=None,
                    **kwargs
                ...
                @action()
                async def subscribe_to_classrooms_in_school(
                    self,
                    school_pk,
                    request_id,
                    **kwargs
                    ...
                    await self.classroom_change_handler.subscribe(
                        school=school,
                        request_id=request_id
                    )

                @action()
                async def subscribe_to_classroom(
                    self,
                    classroom_pk,
                    request_id,
                    **kwargs
        ...
        It is important that a corresponding :meth:`groups_for_signal`
        method is provided that enumerates the groups
    ...
    def groups_for_signal(self, func: Callable[..., Generator[str, None, None]]):
        ...
        .. note::
            Should be used as a method decorator
            eg: `@observed_handler.groups_for_signal`


        The decorated method is used
        whenever an event happens that the observer is observing
        (even if nothing is subscribed).

        The role of this method is to enumerate the groups
        that the event should be sent over.
        ...
        It is important that a corresponding :meth:`groups_for_consumer`
        method is provided to enable the consumers to
